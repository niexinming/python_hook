    ...
    def __getattribute__(self, name):
        ...
        if isinstance(retval, collections.Callable):
            try:
                ...
                for fcn_template_name, fcn_template_code in _HOOK_TEMPLATES_DICT.items():
...
class _InstallFcnHook(object):
    ...
    def _pre_hook(self, *args, **kwargs):
        print("hook")
        print(args)
        with io.open('/tmp/subprocess.log', 'a+') as file:
            file.write(str(args)+"\n")
        return (args, kwargs)
...
Popen=_InstallFcnHook(Popen, debug=True)

call=_InstallFcnHook(call, debug=True)
